...
- **The VM runs under a persistent user slice foldyard owns** (`fy-machine-<vm>.slice`,
  `WantedBy=default.target`). A slice survives being emptied, so its cgroup ID — the one the
  operator's table binds to — is the same across every VM restart. It is the one *user-level*
  change the wall makes, and it follows the courtesies Linux expects of a tool writing into
  `$HOME`: the unit lives in **`$XDG_DATA_HOME/systemd/user/`** (systemd.unit(5): "units of
  packages that have been installed in the home directory" — not `~/.config/systemd/user/`,
  which is for units the operator wrote), it carries a `# Generated by foldyard` header naming
  the verb that regenerates it and the line that removes it, and **only `fy machine host-wall`
  creates and enables it, saying so once when it does** — the same shape as rootless Docker's
  `dockerd-rootless-setuptool.sh install`, a setup verb, never a side effect of a run verb. A
  launch verb only checks that the slice is active and refuses *before booting* when it is not
  (a transient slice from `systemd-run --slice` would be a new ID the table does not hold).
  foldyard never stops it.
- **`fy machine host-wall` renders the root-side files into the project's state dir and prints
  ...
  `enable --now`. `--uninstall` prints the three root lines that remove them and the two
  user-level lines that remove the slice (last, since `--now` stops it). foldyard runs none of
  them.
