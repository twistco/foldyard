...
| [0026](./0026-vscode-attach-config-is-declarative.md) | The VS Code attached-container config is declarative `[vscode]` data (`extensions`, `settings`) read from the ADOPTED copy behind the gate; `remoteUser` + the port pin as facts; the in-box generator script and the mount read of `.vscode/extensions.json` are removed — amends [0023](./0023-no-host-executed-code-from-the-repo-mount.md) §2 |
