...
| [0011](./0011-machine-backends-one-socket-contract.md) | Machine backends behind one libpod-socket contract: podman / lima (the third, native, retired by [0026](./0026-always-a-vm-native-backend-retired.md)) |
...
| [0026](./0026-always-a-vm-native-backend-retired.md) | foldyard always has a VM: the `native` (no-VM) backend retired — its CI rationale replaced by the Lima/QEMU runner job, WSL2 does not rescue it, and every product claim needs the VM; a config naming it gets a loud warning + the podman backend |
