...
| the `③` question — a per-box microVM under the machine VM | measured, **deferred**: libkrun costs 25× on `git status`; gVisor ~3× and is the front-runner (rootless, no KVM) | 2026-09-11 / 12 | [isolation-layers](./isolation-layers.md#the-outs-measured) |
