...
| the `③` question — a per-box userspace kernel (gVisor) under the machine VM | measured, **deferred**: libkrun 25× on `git status`; gVisor ~3× on micro git walks (x86 rig AND Mac arm64/vz). **As the real box runtime** (foldyard's own box args, rootless runsc in the guest): `fy verify` ALL PASS in it; sustained — test suite 1.6×, `uv sync` 2.5×, fork/exec 11×, git walks ~1×. `--ignore-cgroups` free; `host-uds=all` is the unfiltered socket so narrowing is a precondition; inotify-inward needs polling. **Blocker for a product knob:** podman's API has no per-container runtime selection at 5.8 (`--runtime` is local-only; `oci_runtime` honoured from 6.0 on the libpod endpoint only) | 2026-09-11 / 12 / 13 | [isolation-layers](./isolation-layers.md#gvisor-as-the-real-box-runtime-and-a-sustained-build-2026-09-13-fourth-rig-session) |
...
- **`③` on Linux**: gVisor is the candidate; its cost is measured on micro-loops AND as the real
  box under a sustained build (the ③ row above). `--ignore-cgroups` and `host-uds` are settled.
