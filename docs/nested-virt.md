...
- **Select libkrun persistently** in `~/.config/containers/containers.conf`:
  ...
  #9860 makes them appear broken). The default provider is still `applehv`, which does **not**
  expose `/dev/kvm` — so opting into libkrun is required.
...
- **Whether nested VMs behave better under a Lima machine** (QEMU/VZ) than under
  ...
  starve less under a real guest workload. Needs a Lima-backend host to try — it can't be
  tested from a box running on the podman machine.
