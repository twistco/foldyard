...
class DevModeTui(App):
    ...
    def _start_devbox(self, ws: dict) -> None:
        ...
        def _after(answer: str | None) -> None:
            ...
            try:
                ...
                # the tree moved while the question was open, so ask again over the new tree.
                configpin.adopt(drift.cfg, reviewed=drift.tree_digest())
    ...
    def _refresh_manage_panel(
        ...
        if not pane:
            return
        self.query_one("#network-manage-summary", Static).update(Text.from_markup(summary))
        sig = tuple(rows)
