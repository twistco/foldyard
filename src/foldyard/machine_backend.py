...
class LimaBackend(Backend):
    ...
    def _set_expr(self, resources: dict, volumes: list[tuple[str, str]]) -> str:
    ...
    def create(self, name: str, resources: dict, volumes: list[tuple[str, str]]) -> bool:
        """SPIKE: create (stopped) from the podman template, overriding sizing + mounts via
        ``--set``. Keeps the template's podman provisioning + socket forward; ``--tty=false``
        skips the interactive review."""
        cmd = [
            "limactl",
            "create",
            "--tty=false",
            ...
            "template://podman",
            "--set",
            self._set_expr(resources, volumes),
        ]
