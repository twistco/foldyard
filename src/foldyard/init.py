...
def render(opts: InitOptions) -> str:
    ...
    lines = [
        ...
        *_version_lines(opts.version),
        ...
        "#   host_wall = true    # Linux hosts: enforce the same wall on the HOST too (nftables;",
        "#                       # you install it once — `fy machine host-wall` prints the steps)",
        ...
        '# request (the `passthrough` toolchain aside). `default_deny = "learn"` starts the wall',
        ...
        "#     fy allow wall learn --for 30m         # learn again, e.g. for a new dependency",
        "#     fy allow wall on                      # enforce now",
        "# (`default_deny = true` enforces from the first run instead; the recommendations below",
