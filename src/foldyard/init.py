...
def render(opts: InitOptions) -> str:
    ...
    lines = [
        ...
        "#      allowlisting proxy, ONLY this repo mounted, no push credential. `fy verify` proves",
        "#      the cage. The first `fy box up` starts the wall LEARNING for an hour: setup isn't",
        "#      refused, every host it would have refused is recorded, and then it ENFORCES by",
        "#      itself. `fy allow learn` grants what it saw in one reviewed batch ([proxy] below).",
        ...
        *_version_lines(opts.version),
        ...
        "#   firewall = false       # lima but COOPERATIVE-only: the box uses the proxy, but",
        "#                          # nothing stops a process that ignores it",
        "#   host_firewall = true   # Linux hosts: enforce the same firewall on your computer too",
        ...
        "#                       # (a kernel-exploit layer; ~1.2x on a test suite; host-side edits",
        "#                       # need dev-server polling — docs/configuration.md)",
        'backend = "lima"',
        "firewall = true",
        f'name = "{prefix}"',
        f"cpus = {opts.cpus}",
        ...
        "# by the wall (which otherwise leaves the box with no way out). It decrypts and logs every",
        '# request (the `passthrough` toolchain aside). `enforce = "learn"` starts the wall',
        "# OBSERVING on the first `fy box up`/`fy up` for an hour — nothing refused, every host it",
        "# would refuse recorded — then ENFORCING, by itself (it can't be left open by mistake):",
        ...
        "# pane manages grants + these offers). `enforce` here only SEEDS the first answer —",
        "# `fy allow enforce` is the switch from then on. Any keyless host (below) is ALWAYS allowed",
        ...
        "# so an enforcing wall comes up working after one round of consented yeses; the agents",
