...
Run it with mitmdump (see the `just egress-proxy` recipe; the supervisor passes the project's
...
class _Rule:
    ...
    the legacy single ``INJECT_HOST``/``INJECT_COMMAND``/… env (the bash recipe + older callers)."""
...
class Injector:
    def __init__(self) -> None:
        ...
        # foldyard proxy plugin emits; else fall back to the legacy single INJECT_HOST/COMMAND/…
        # env (one rule), so the bash `just egress-proxy` recipe + older callers still work.
        ...
        # any caller (e.g. the bash `just egress-proxy` recipe) that doesn't set CAPTURE_MODE.
