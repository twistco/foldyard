...
```text
 container (app / dev box)                in the VM                      your computer
 ┌──────────────────────────┐     ┌───────────────────────────┐   ┌────────────────────┐
 ...
 │   → believes it's on GCE  │◀────│  → asks the host to mint   │◀──│  gcloud creds HERE │
 └──────────────────────────┘     └───────────────────────────┘   └────────────────────┘
```
...
- **`server.py`** runs as a container ON the stack network (the consumer's compose
  ...
  container's `gcp.serviceAccount` label, then fetches a token from the minter on
  your computer, caching per-SA. Source-IP resolution _must_ live on-network: a
  container→host request is NAT'd through the VM's network stack, so your computer only ever sees the gateway IP.
- **`minter.py`** runs on **your computer**, where the gcloud credentials live. It validates the
...
- Each container declares its identity with a **`gcp.serviceAccount` label**. Resolution
  ...
  TTL-bound `gcp=user` emergency level reachable for the box and no one else.
...
key list, and `docs/modes.md` for what each level grants.
...
The level decides which of those SAs the minter will allowlist:
...
1. **The SAs exist**, and each has exactly the roles its level is meant to grant — the
   `box` SA in particular should be read-only, since a dev box holding it is the normal
   resting state under `gcp=logs`.
2. **Your own identity holds `roles/iam.serviceAccountTokenCreator` on every allowlisted
   SA.** That grant is the whole trust chain: minting is impersonation, so if you
   don't have it you get a clear gcloud error rather than a token. If your organisation
...
```bash
fy mode gcp=logs     # or gcp=sa / gcp=user ttl=30m   (your computer only — the mode is host-side)
...
the level, so there is nothing to keep in sync by hand. Explicit env still wins if you need
...
levels that talk to real infrastructure. Compose `environment:` values can't be unset from
an env-file, so that belongs in a mode overlay (`[[overlay]]`, keyed on the level) —
...
- gcloud credentials never leave your computer; only ≤1h impersonated tokens cross the boundary.
