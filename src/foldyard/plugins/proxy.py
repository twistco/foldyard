...
class ProxyPlugin(Plugin):
    ...
    def daemons(self, mode: dict) -> dict[str, dict]:
        ...
        if len(rules) == 1:
            # ONE injector: emit the legacy single INJECT_* env (byte-identical to before — the bash
            # `egress-proxy` recipe + every single-rule caller read these keys). The addon uses
            # them when INJECT_RULES is empty. Its label already names the credential.
            rule = rules[0]
            env = {
                **base_env,
        ...
        return {
            ...
            # capture axes map to this daemon, and the bash `fy box`/`egress-proxy` recipes + the
            # addon read these exact keys. For github the env equals egress_proxy.py's defaults.
    ...
    def box_args(self, env: dict) -> list[str]:
        ...
        if not ca.exists():
            raise SystemExit(
                f"✗ FY_PROXY set ({proxy}) but no CA at {ca} — run 'fy host' (or 'fy "
                "egress-proxy') ON THE MAC once to generate it."
            )
