#!/usr/bin/env python3
"""The host firewall (``[machine] host_firewall``) for the machine VM — the zero-cost hardening
step above ``③`` on a LINUX host (see docs/isolation-layers.md, "Host firewall on Linux").
...
"what can this host do?" question the backend's vmtype resolution asks — not "is this macOS?".
...
``fy machine host-firewall`` writes and enables (:func:`ensure_slice` — the wall's one user-level
...
**How :mod:`foldyard.machine` wires it (``[machine] host_firewall = true``).** The VM is STARTED
...
def slice_unit_text(vm: str) -> str:
    ...
    return f"""\
# Generated by foldyard (`fy machine host-firewall`) — do not edit; the verb regenerates it.
# Remove with: systemctl --user disable --now {slice_unit(vm)}
[Unit]
Description=foldyard machine VM '{vm}' (the cgroup the host firewall matches)
...
def service_unit_text(vm: str, uid: int, nft: str) -> str:
    ...
    return f"""\
[Unit]
Description=foldyard host firewall for machine VM '{vm}' (uid {uid})
Documentation=https://github.com/twistco/foldyard/blob/main/docs/configuration.md
...
def ensure_slice(vm: str) -> str:
    ...
    change foldyard makes for the wall, and only ``fy machine host-firewall`` makes it: ``fy up``
...
class Probe:
    ...
    def detail(self) -> str:
        ...
        for name, got in self.checks.items():
            ...
            parts.append(f"{_LABEL.get(name, name)} {'✓' if good else '✗'} {got}")
        return ", ".join(parts)
...
# How a check reads in that line (the keys are the probe child's JSON, so they stay put).
_LABEL = {"band": "port range"}
