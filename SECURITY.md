...
foldyard is a security tool: its job is to keep what a compromised agent or package can reach
inside a throwaway VM, and to keep credentials on your computer. A bug in that boundary matters more than any feature.
...
host platform you were on (`fy --version`). We will acknowledge the report and keep you informed while we work on it.
...
- **Egress** — traffic leaving the box that bypasses the proxy or the VM firewall, or an
  enforcing allowlist (`fy allow enforce on`) that lets an unlisted host through.
- **Config trust** — any path by which editing `foldyard.toml` in the checkout changes what the
  host does without you adopting it (see ADR-0022 and ADR-0023 in `docs/adrs/`).
