...
async def test_the_adopt_modal_shows_what_the_branch_changes_about_mains_config(monkeypatch):
    ...
    async with tui.DevModeTui().run_test() as pilot:
        ...
        # The diff sits in a scroll pane, which takes the modal's auto-focus (so ↑/↓ review a long
        # branch) — the answer keys still have to reach the screen's bindings past it.
        await pilot.press("a")
        await pilot.pause()
